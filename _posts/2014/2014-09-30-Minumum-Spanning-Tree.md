...
We add the edge e to the minimum spanning tree T and this creates a unique simple cycle C. Let \\(e_{max}\\) be an edge of maximum weight in the cycle C. Then, removing this edge would give us a spanning tree T'=T∪{e}{\\(e_{max}\\)} since we would have |V|-1 edges and no cycles. To prove that T' is a minimum spanning tree for the new graph G', we consider two distinct cases.
